...
（與 by-tag 搜尋並列的第二個檢視維度）。**交付面：win-x64 自包含單檔 exe 已實測就緒**（見 [`docs/deployment.md`](docs/deployment.md)）。
...
- ✅ **瀏覽器層 e2e（Playwright）**：`src/Pm.Web/e2e/browse-smoke.mjs` 起真實 app + 瀏覽器層 mock `/api`，驗證捲動自動補頁、切夾無交叉污染（`npm run e2e`）
