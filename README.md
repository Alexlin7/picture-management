...
tag 顯示層、樣式系統、頂端操作重構、async scan、**作品軸**、logging 收斂、孤兒清理、**資料夾路徑維度瀏覽 `/browse`**
（與 by-tag 搜尋並列的第二個檢視維度），以及**單張重新處理 + 掃描自動痊癒**與**前端 RWD（桌面縮放韌性 + 手機 topbar/toolbar）**。
**交付面：win-x64 自包含單檔 exe 已實測就緒**（見 [`docs/deployment.md`](docs/deployment.md)）。
...
- ✅ **PR #5 review 後續修復（2026-06-26）**：`/code-review high` 的 10 條 findings（切夾競態、log 級別 fallback、懸空 photo_tag、自動分頁停擺、裸 hex token 化、共用 helper 抽取…）**全部驗證 + 修復**（後端 145 綠 + 前端 81 綠 + 瀏覽器 e2e 全過；詳見 `docs/superpowers/specs/2026-06-26-pr5-folder-browse-review-and-test-plan.md`）
- ✅ **瀏覽器層 e2e（Playwright）**：`browse-smoke.mjs`（捲動自動補頁、切夾無交叉污染，`npm run e2e`）＋ `rwd-resize-smoke.mjs`（縮 viewport 斷言無破版 + 欄數遞減，含手機寬度 480/375，`npm run e2e:rwd`）
- ✅ **單張重新處理 + 掃描自動痊癒**：`POST /api/photos/{id}/reprocess`（重新解碼補 metadata + 強制重產縮圖 + refresh WD14）；重掃自動痊癒 `width=NULL` 半殘圖（AVIF 解碼上線前索引的舊檔）；inspector「重新處理」鈕,完成後縮圖即刷新（cache-bust）
- ✅ **前端 RWD（桌面縮放韌性 + 手機 topbar/toolbar）**：共用 `<app-masonry>` 瀑布流（JS 量測欄寬、aspect 驅動、最少 1 欄）；facet / inspector / 資料夾樹側欄可收合（手動 + 依斷點自動）；窄寬（≤640px）topbar 折行 + 隱藏冗餘提示、toolbar 採「⋯ 更多」溢出選單收次要操作
- 🚧 推論後端：CPU / DirectML 可用；CUDA、Windows ML 僅骨架（見 `src/Pm.Ml`）
- ✅ **AVIF / HEIC / HEIF 已支援**：ImageSharp 3.x 不解 HEIF 家族,故新增 `Pm.Imaging` 橋接 —— 這族繞道 **Magick.NET（內建 libheif）** 解成像素再包回 ImageSharp,縮圖/尺寸/MIME/WD14 自動標皆正常；其餘格式仍走 ImageSharp（HEIC 為解碼支援,Magick-Q8 無 HEIC 編碼但本專案只需解碼）
...
> **dev 與 release 的預設不同**：`appsettings.json` 出廠**預設關**（release / 攜帶版以此為準）；但 dev 用的 `Properties/launchSettings.json`（Pm.Api profile）已固定帶 `Inference:Wd14:Enabled=true`，所以 `dotnet run --project src/Pm.Api` 走 launch profile 時 **WD14 是開的**（方便開發實測）。要完全不載模型，用 `--no-launch-profile` 或把該值改 `false`。
...
cd src/Pm.Web ; npm run e2e:rwd   # RWD resize e2e（需先起 app;縮 viewport 斷言無破版）
```
...
- 相簿：搜尋為 **下拉驅動**（打片段挑既有標、AND 隱含、點 chip 切排除、精準 Enter、查無此標、已選標不重複）；「儲存搜尋」已接（存+套用）、「重標失敗」批次 requeue 已接、冗餘掃描鈕已刪；總命中數（`/api/search/count`）與 WD14 待標佇列數（`/api/tagging/stats` 每 4s 輪詢）皆顯真實值；per-tile tag chips 移除
