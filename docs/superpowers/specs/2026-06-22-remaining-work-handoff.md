...
1. **掃描效能 + tagging 解耦**
   ...
   - Slice 3 已完成:`POST /api/tag/requeue` + `POST /api/photos/{id}/retag`,支援 `retry` / `refresh` / `clear` 與 `photoIds` / `error` / `root` / `all` scope;job upsert、refresh/clear 只清該批 `source='wd14'` tag、root/all 只挑 present、大量 IN 分塊。
   - 下一步:Slice 4「WD14/CLIP 能力開關拆分」。
