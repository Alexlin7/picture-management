...
- 空的 kind 區不顯示。
- **窄側欄版面(2026-06-24 實作補):** 檢視器是窄側欄,character 標(label + ‹造型›/‹作品› 徽章 + 長 canonical + 來源)單行會橫向爆寬。故**有造型/作品徽章的 tag 改「兩行卡片」**:主行 `emoji + label + ‹造型:…›/‹作品:…›`,副行(灰、小)`canonical(僅當 ≠ label)· 來源%` + 移除鈕;**canonical 仍永遠可見可搜**(降為副行,未隱藏,守原則)。無徽章的短標(general/expression/meta/manual/path、或無 work 的 character)維持**單行 chip**。純版面、`displayOf`/`groupTags` 邏輯不動。
